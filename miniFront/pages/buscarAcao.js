    ...
    const btn = document.getElementById('buscarStock');
    if (btn) {
      btn.addEventListener('click', function(event) {
        event.preventDefault();
        const ticker = document.getElementById('bticker').value;
        console.log('Ticker a ser buscado:', ticker);
        if (!ticker) {
          document.getElementById('responseDiv').innerText = 'Por favor, insira um ticker.';
          return;
        }
  
        let url = 'http://localhost:8080/api/v1/stocks/ticker/' + ticker;
